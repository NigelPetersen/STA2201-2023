...
```{r}
dm_1 <- dm |> 
      mutate(Ratio = Male/Female) |>                                      filter(Age==10| Age==20 | Age==30 | Age==40)

dm_1 |> ggplot(aes(x = Year, y = Ratio, col = Age)) + 
  geom_line()
```
...
```{r}
dm_2 <- dm |> group_by(Year) |> select(Year, Age, Female) |>
    mutate(max_val = max(Female, na.rm = TRUE))

dm_max_age <- dm_2 |> group_by(Year) |> 
  filter(Female == max_val) |>
     select(Year, Age)
...
```{r}
dm_3 <- dm |> group_by(Age) |>
  summarise(across(Female:Total, sd, na.rm = TRUE)) |>
  arrange(Age)
dm_3

# I'm not sure why the ages are not in increasing order
